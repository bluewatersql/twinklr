...
@register_template(aliases=["Reverse Every Other", "reverse every other"])
    ...
    return TemplateDoc(
        template=Template(
            ...
            metadata=TemplateMetadata(
                description="Phase order reverses every other cycle via PING_PONG mode.",
                recommended_sections=["build", "chorus"],
                energy_range=(55, 90),
                tags=["repeat", "phase_offset", "reverse"],
            ),
        ),
        presets=[
            TemplatePreset(
