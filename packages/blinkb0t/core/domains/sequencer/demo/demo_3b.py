...
def main() -> None:
    rig = build_demo_rig()
    template = reverse_every_other.TEMPLATE
    ...
    iteration_policy = ReverseEveryOtherPolicy()
    ...
    print("\n=== DEMO 03B: Reverse Every Other Cycle ===\n")
    print("Template:", template.template_id)
    print("Window: 16 bars @ 120bpm (4 iterations)")
    print("Adjustments: phase order alternates LEFT_TO_RIGHT / RIGHT_TO_LEFT\n")
    ...
    for i in range(total_cycles):
        ...
        order = step_doc.timing.phase_offset.order
        print(f"cycle {i + 1}: phase_order={order.value}")
    ...
    print("\nTip: compare per-fixture t0/t1 across cycle boundaries.\n")
