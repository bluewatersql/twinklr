...
def main() -> None:
    ...
    template = wave_fan_hold.TEMPLATE
    segments = compiler.compile(rig=rig, plan=plan, template_doc=template)
    print("\n=== DEMO 11A: Fan Wave + Hold ===\n")
    print("Template:", template.template_id)
    _print_segments("11A", segments)

    template = wave_scattered_fade_in.TEMPLATE
    segments = compiler.compile(rig=rig, plan=plan, template_doc=template)
    print("\n=== DEMO 11B: Scattered Wave + Fade In ===\n")
    print("Template:", template.template_id)
    _print_segments("11B", segments)

    template = wave_scattered_fade_out.TEMPLATE
    segments = compiler.compile(rig=rig, plan=plan, template_doc=template)
    print("\n=== DEMO 11C: Scattered Wave + Fade Out ===\n")
    print("Template:", template.template_id)
    _print_segments("11C", segments)
