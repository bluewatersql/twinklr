...
def main() -> None:
    ...
    template = circle_fan_hold.TEMPLATE
    segments = compiler.compile(rig=rig, plan=plan, template_doc=template)
    print("\n=== DEMO 08A: Fan Circle + Hold ===\n")
    print("Template:", template.template_id)
    _print_segments("08A", segments)

    template = circle_asym_right_pulse.TEMPLATE
    segments = compiler.compile(rig=rig, plan=plan, template_doc=template)
    print("\n=== DEMO 08B: Asym Right Circle + Pulse ===\n")
    print("Template:", template.template_id)
    _print_segments("08B", segments)

    template = circle_asym_left_strobe.TEMPLATE
    segments = compiler.compile(rig=rig, plan=plan, template_doc=template)
    print("\n=== DEMO 08C: Asym Left Circle + Strobe ===\n")
    print("Template:", template.template_id)
    _print_segments("08C", segments)
