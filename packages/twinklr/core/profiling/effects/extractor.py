...
from twinklr.core.profiling.identity import (
    EFFECT_EVENT_NAMESPACE,
    canonical_json,
    content_uuid5,
)
...
def _config_fingerprint(config: dict[str, Any]) -> str:
    return hashlib.sha1(canonical_json(config).encode("utf-8")).hexdigest()
...
def extract_effect_events(
    ...
    # Counts prior events sharing an identical signature, so two genuinely
    # distinct effects with the same start/end/layer/target/type/config still
    # get distinct ids. Append order is the fixed nested-loop order below.
            ...
            for effect in layer.effects:
                ...
                signature = ":".join(
                    (
                        package_id,
                        sequence_file_id,
                        str(effect.start_time_ms),
                        str(effect.end_time_ms),
                        layer_name,
                        target_name,
                        effect.effect_type,
                        config_fingerprint,
                ...
                dup_index = signature_counts.get(signature, 0)
                signature_counts[signature] = dup_index + 1
                events.append(
                    EffectEventRecord(
                        effect_event_id=content_uuid5(
                            EFFECT_EVENT_NAMESPACE, f"{signature}:{dup_index}"
                        ),
