...
def extract_beat_sync_features(
    ...
    Args:
        ...
        y_harm: Pre-computed harmonic component from HPSS (optional). If provided,
    ...
    if onset_env is not None:
        onset_env_2d = onset_env.astype(np.float32).reshape(1, -1)
    else:
        onset_env_2d = (
    ...
    onset_sync = librosa.util.sync(onset_env_2d, beat_frames, aggregate=np.mean)[:, :num_beats]
